// weather app
let searchInputBox=document.getElementById("search_inputbox");
...
searchIconContainer.addEventListener("click",async ()=>{
    ...
    weatherAPIURL=`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=46b4aedd0176e6ab5b289442a68d32f1`;
    ...
    document.getElementById("weather_temperature_value").textContent=parseInt(weatherReport.main.temp-273.15);
    document.getElementById("humidity_status_value").textContent=`${weatherReport.main.humidity}%`;
    document.getElementById("wind_speed_value").textContent=`${weatherReport.wind.speed}km/hr`;
